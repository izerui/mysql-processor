...
class MyDump(BaseShell):
    ...
    def _export_tables_data(self, database: str, dump_file: str) -> int:
        ...
        # 使用tqdm显示进度条
        with tqdm(total=len(tables), desc=f"{Fore.MAGENTA}📊 并行[{self.threads}]导出: [{database}] 数据库", unit="表",
                  dynamic_ncols=True, disable=False,
                  file=sys.stdout, ascii=True, miniters=1, mininterval=0.1, position=0, leave=True) as pbar:
